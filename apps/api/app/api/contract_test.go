...
func TestCommittedContractMatchesGenerator(t *testing.T) {
	var generated bytes.Buffer
	encoder := json.NewEncoder(&generated)
	encoder.SetEscapeHTML(false)
	encoder.SetIndent("", "  ")
	if err := encoder.Encode(NewContractAPI().OpenAPI()); err != nil {
		t.Fatalf("generate OpenAPI: %v", err)
	}
	committed, err := os.ReadFile("../../openapi/vocanova.openapi.json")
	if err != nil {
		t.Fatalf("read committed OpenAPI: %v", err)
	}
	if !bytes.Equal(committed, generated.Bytes()) {
		t.Fatal("committed OpenAPI drifted; run go run ./cmd/openapi and review the semantic diff")
	}
}
