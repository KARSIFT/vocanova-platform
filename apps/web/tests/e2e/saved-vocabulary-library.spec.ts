...
test("offers Journey discovery when the saved library is empty", async ({
  ...
  const hundredAstralCharacters = "😀".repeat(100);
  await search.fill(`${hundredAstralCharacters}😀`);
  await expect(search).toHaveValue(hundredAstralCharacters);
...
test("searches saved meanings by normalized definition, focuses results, and clears a no-match filter", async ({
  ...
  await expect(
    page.locator('a[href="/discover/saved/bank?meaning=mean-bank-money"]'),
  ).toBeVisible();
