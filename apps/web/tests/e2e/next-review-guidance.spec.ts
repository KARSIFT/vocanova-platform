...
test("shows the learner-local scheduled review time on every scheduled-review surface", async ({
  ...
  await expect(
    page.getByText("Next review: Aug 22, 2099, 8:30 AM GMT-04:00"),
  ).toBeVisible();
...
test("keeps scheduled review guidance when a stored timezone is invalid", async ({
  ...
  await page.goto("/reviews");
  await expect(
    page.getByText("Your next review is Aug 22, 2099, 12:30 PM GMT+00:00."),
  ).toBeVisible();

  await page.goto("/discover/saved");
  await expect(
    page.getByText("Next review: Aug 22, 2099, 12:30 PM GMT+00:00"),
  ).toBeVisible();
});
