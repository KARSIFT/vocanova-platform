...
describe("Worker content, learning, and review parity", () => {
  ...
  it("searches only the owner's saved meanings with a normalized, query-bound cursor", async () => {
    ...
    const legacyUnfilteredCursor = btoa(
      JSON.stringify({
        a: "2026-08-22T14:00:00.000Z",
        i: USER_WORD_B,
      }),
    )
      .replace(/\+/g, "-")
      .replace(/\//g, "_");
    await expect(
      repository.listSavedWords(USER_A, legacyUnfilteredCursor, 10),
    ).resolves.toMatchObject({
      items: [
        expect.objectContaining({ meaningId: MEANING_C }),
    ...
    await expect(
      repository.listSavedWords(USER_A, "", 10, "😀".repeat(51)),
    ).resolves.toMatchObject({ items: [] });
    await expect(
      repository.listSavedWords(USER_A, "", 10, "%_"),
    ).resolves.toMatchObject({ items: [] });
    const unicodePage = await repository.listSavedWords(USER_A, "", 1, "☕");
    expect(unicodePage.items.map((item) => item.meaningId)).toEqual([
      MEANING_C,
    ]);
    await expect(
      repository.listSavedWords(USER_A, unicodePage.nextCursor!, 1, "☕"),
    ).resolves.toMatchObject({ items: [] });
    expect(
  ...
  it("serializes concurrent saves for one learner meaning without exposing a uniqueness failure", async () => {
    ...
    const activeWords = await env.DB.prepare(
      ...
      .first<{ count: number }>();
    const activeWord = await env.DB.prepare(
      "SELECT id FROM user_words WHERE user_id = ?1 AND meaning_id = ?2 AND deleted_at IS NULL",
    )
      .bind(USER_A, MEANING_A)
      .first<{ id: string }>();
    ...
    expect(saves[0].userWordId).toBe(activeWord?.id);
    expect(saves[1].userWordId).toBe(activeWord?.id);
    expect(saves[0].userWordId).toBe(saves[1].userWordId);
