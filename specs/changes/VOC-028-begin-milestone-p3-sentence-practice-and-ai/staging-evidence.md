...
`VOC-028-AC-09` and `VOC-028-T05`. It is produced under the (proposed)
resolutions `VOC-028-D00` through `VOC-028-D06` once adopted. This is a
**draft** and does not declare the DOC-12 P3 milestone gate complete.

Live staging exercises (EV-28, EV-29, EV-30) and protected live-model
provider evaluation are blocked by two open dependencies:
...
- `VOC-028-DEP-02` (`D02`): the production provider/model + privacy decision is
  open; the production adapter PR (T02) cannot be accepted and no protected
  live-model evaluation can run until it is resolved and recorded (DOC-09 §18,
  §21, §24; DOC-12 §5).
...
| `EV-22`..`EV-27` | Evaluation fixtures, golden set, CI-pays-free, privacy-safe observability, AI-disable seam, change metadata | Ready | ≥200-case dataset + `golden-set-v1`; metrics without learner text; AI-disable keeps non-AI features working. |
| `EV-31` | Installed deterministic/security suite + extended mock-inventory | Ready | `pnpm validate`/`test`/`build`, Go format/vet/test/build, `scripts/governance/*` as applicable, extended `mock-inventory.mjs`. |
...
5. Once `D02` resolves, repeat with the real provider under the protected
   dev/staging evaluation + cost ceiling and record latency/cost.
...
## Provider evaluation + privacy verification (blocked by `D02`)

Before production, evaluate providers in the DOC-09 §18 order and verify each
provider's training-data use, retention, processing regions, subprocessors, and
deletion procedures; prefer configurations where API content isn't used for
training and retention is disabled/minimized (DOC-09 §21). Record the chosen
provider/model/config, the privacy review result, the secrets-provisioning plan
(secrets stay backend-only), the evaluation scores against the MVP acceptance
thresholds (DOC-09 §23), and the cost/ceiling plan. This is recorded as a
placeholder until `D02` resolves.
...
- `VOC-028-DEP-02` (`D02`): provider/model + privacy decision is open, so the
  production adapter acceptance, protected live-model evaluation, and privacy
  verification are recorded as placeholders until resolved. CI uses the mock
  provider only.
...
- Resolve `D02`, then accept T02 and run protected offline live-model
  evaluation + the real-provider staging exercises.
- P4: replace the `D01` mission-completion stub with real
  `daily_mission_snapshots`/streak/point wiring once those tables exist.
- `D03`/`D04`: set AI-disable/cost-ceiling activation values and retention
  defaults (legal review) before production.
