...
URL/header injection, unsafe avatar, mixed-capability coupling, production timeout-
factory drift, email observability/parity regression, identity-link/session corruption,
cross-user access, contract/schema drift, runbook-policy failure, or failed required
checks. Revert removes only the repository delta; no D1 down migration or external
action is needed because no schema or live state changed.
