...
## VOC-018-T00 — Make `@vocanova/design-tokens` importable and depend on it from apps/web

- Requirement source: `VOC-018-D00`
- Acceptance criteria: `VOC-018-AC-00`
- Tests: `VOC-018-TEST-00`, `VOC-018-TEST-04`
...
## VOC-018-T01 — Emit the eight scales as Tailwind v4 @theme custom properties in apps/web

- Requirement source: `VOC-018-D01`, `VOC-018-D02`, `VOC-018-D04`
- Acceptance criteria: `VOC-018-AC-01`, `VOC-018-AC-02`
- Tests: `VOC-018-TEST-01`, `VOC-018-TEST-02`
- Evidence: `VOC-018-EV-01`
- Status: pending

Recommended (generation, `VOC-018-D00`):
...
1. Add `apps/web/scripts/generate-tokens-css.mjs`: import the eight scales from
   ...
   key per the `VOC-018-D01` table (colors incl. `neutral`/`primary`/`secondary`,
   `--spacing-*`, `--text-*`, `--radius-*`, `--shadow-*`, `--ease-*`, and the
   `--duration-*` custom properties). Emit exactly the 64 properties in
   `VOC-018-AC-01`; do not invent keys or line-heights.
...
Alternative (hand-authored, if the founder selects it in `VOC-018-D00`): write
...
Tailwind defaults are `VOC-018-D01`/`D02` and may be amended at adoption
(`VOC-018-DEP-04`); implement the confirmed choice. Do **not** add any route,
page, or component; `page.tsx`/`layout.tsx` stay unchanged.

## VOC-018-T02 — Add the token↔CSS drift check, enforced by CI without a workflow edit

- Requirement source: `VOC-018-D00`, `VOC-018-D03`
- Acceptance criteria: `VOC-018-AC-03`, `VOC-018-AC-04`
- Tests: `VOC-018-TEST-03`, `VOC-018-TEST-04`
...
1. Add `scripts/foundation/tokens-css.test.mjs` (a `node:test` file, matching the
   ...
   per the `VOC-018-D01` mapping. For the generation design, it additionally
...
2. Because the root `test` script is
   ...
   `VOC-018-D03`). Document the ordering dependency (`dist` is emitted by the
