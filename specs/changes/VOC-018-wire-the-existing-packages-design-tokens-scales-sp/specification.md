# VOC-018 — Wire design-token scales into apps/web (Tailwind v4 @theme): Specification
...
3. **Drift enforcement** — a deterministic check that fails when the emitted CSS
   ...
   already runs (`VOC-018-D03`), with **no `.github/workflows/*` edit**.
...
scale (none exists — `VOC-018-DEP-05`); Tailwind plugin authoring; shadcn/ui
...
design deliberately avoids it (`VOC-018-D03`). No R3/R4 protected path is touched
...
> tracked by `VOC-018-DEP-04`. Confirm or amend them at adoption — the task,
> test, and file structure are stable across the alternatives.

### `VOC-018-D00` — Single source of truth: generate, don't hand-copy (OPEN)
...
Either way a **drift check** (`VOC-018-D03`) binds the CSS to the objects. If the
founder prefers minimal tooling, the hand-authored variant is acceptable: the
generator task (`VOC-018-T01`) then produces a hand-written
...
regenerated output*. Only `VOC-018-T01`'s mechanism changes; `VOC-018-T00`,
`VOC-018-T02`, the mapping table, and all acceptance criteria are unaffected.

### `VOC-018-D01` — Namespace mapping (OPEN on the flagged rows)
...
| `neutral` | `50…900` | `--color-neutral-<key>` | `bg-neutral-100`, `text-neutral-900`, `border-neutral-200` | **Overrides** Tailwind's built-in `neutral-*` values with the token values (`VOC-018-D02`). |
...
### `VOC-018-D03` — Where the drift check runs (no workflow edit)
...
in `implementation-plan.md` and tracked as `VOC-018-R02`.

### `VOC-018-D04` — Duration consumption pattern

Because there is no `duration-<name>` utility (`VOC-018-D01`), the emitted
