...
first breaking risk is cosmetic only (see `VOC-018-R00`). Rollback is a plain
...
- `VOC-018-R00`: Low, cosmetic. Overriding `--color-neutral-*`, `--text-*`,
  `--radius-*`, `--shadow-*`, `--ease-*` changes the values of the corresponding
  Tailwind default utilities (`VOC-018-D02`, merge semantics). `apps/web` today
...
- `VOC-018-R01`: Tailwind v4 namespace behaviour for **named `--spacing-*`
...
- `VOC-018-R02`: Build-order dependency. The drift test imports
  ...
  header (`VOC-018-D03`). If this proves fragile, the fallback is to have the test
  import the token source through the same build the workspace already produces
  rather than adding a transpiler dependency.
- `VOC-018-R03`: Protected-surface risk. Enforcing the drift check by editing
  ...
  existing root `test` glob instead (`VOC-018-D03`); the reviewer must confirm no
  workflow file was touched.
- `VOC-018-DEP-01`: Requirement must reach a founder-approved,
...
- `VOC-018-DEP-02`: `base_sha` to be pinned to the then-current `develop` head at
  adoption.
- `VOC-018-DEP-03`: Depends on the VOC-010→VOC-015 scales (`spacing`, `neutral`,
...
- `VOC-018-DEP-04`: The open design decisions — generate vs hand-author
...
- `VOC-018-DEP-05`: No `feedback` scale exists in `packages/design-tokens`; the
...
- `VOC-018-EV-00`..`VOC-018-EV-04`: CI output (format/lint/typecheck/test/build,
