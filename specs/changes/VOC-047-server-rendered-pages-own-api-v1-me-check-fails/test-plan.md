...
## VOC-047-TEST-00 — Real-request comparison identifies a concrete divergence

- Covers: `VOC-047-AC-00`
- Preconditions: `VOC-047-T00`'s diagnostic logging deployed to a real (or
...
## VOC-047-TEST-01 — Fixed createServerApiClient() succeeds for a session middleware.ts already validated

- Covers: `VOC-047-AC-01`
- Preconditions: `VOC-047-T01`'s fix applied.
- Procedure:
...
## VOC-047-TEST-02 — Regression test fails pre-fix, passes post-fix, runs in CI

- Covers: `VOC-047-AC-02`
- Preconditions: `VOC-047-T02`'s test implemented.
- Procedure:
...
## VOC-047-TEST-03 — Every createServerApiClient() call site is inventoried and covered

- Covers: `VOC-047-AC-03`
- Preconditions: `VOC-047-T03`'s search performed.
- Procedure:
  ...
  3. For each listed call site, confirm it relies on the same
     `createServerApiClient()` implementation `VOC-047-T01` fixed (i.e. no
...
applicable. Negative/authorization coverage: `VOC-047-TEST-01` step 4's
