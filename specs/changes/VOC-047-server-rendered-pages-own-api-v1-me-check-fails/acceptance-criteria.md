# VOC-047 — Acceptance Criteria

## VOC-047-AC-00 — The exact divergence between the two cookie-forwarding paths is identified from a single real request

- Requirement source: issue #333's own suggested diagnostic approach
- Tasks: `VOC-047-T00`
- Tests: `VOC-047-TEST-00`
- Evidence: `VOC-047-EV-00`
- Result: pending
...
## VOC-047-AC-01 — apps/web/src/lib/api-server.ts's createServerApiClient() forwards cookies such that the onboarding page's own /api/v1/me check succeeds for the same session middleware.ts already validated

- Requirement source: issue #333's core reported defect
- Tasks: `VOC-047-T01`
- Tests: `VOC-047-TEST-01`
- Evidence: `VOC-047-EV-01`
- Result: pending
...
## VOC-047-AC-02 — A deterministic regression test catches this specific cookie-forwarding divergence before the next live reproduction is needed
...
- Requirement source: issue #333's implicit ask (three live reproductions were
  ...
  require a fourth)
- Tasks: `VOC-047-T02`
- Tests: `VOC-047-TEST-02`
- Evidence: `VOC-047-EV-02`
- Result: pending
...
## VOC-047-AC-03 — Any other createServerApiClient() call site is confirmed covered by the same fix, or explicitly confirmed to not exist

- Requirement source: specification.md's open question 3 (in scope for this
  package, not deferred)
- Tasks: `VOC-047-T03`
- Tests: `VOC-047-TEST-03`
- Evidence: `VOC-047-EV-03`
- Result: pending
- Observable outcome: a repository-wide search for `createServerApiClient` call
  ...
  `VOC-047-T01`'s fix (since the fix corrects the shared implementation, not a
