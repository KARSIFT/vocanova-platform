...
- Covers: `VOC-090-AC-00` through `VOC-090-AC-04`
- Procedure: inventory all active occurrences of fixed line thresholds, task sizing,
  ...
  count, and multi-PR rationale across the declared files. Classify each occurrence as
  reconcile-active, present-compatible, or historical-preserve. Compare the final
  wording against every `VOC-090-D00` through `D11` decision.
- Expected: all declared active surfaces express one compatible contract, historical
  evidence is not rewritten, and no affected active surface is omitted.
- Evidence: `VOC-090-EV-00`

## VOC-090-TEST-01 — Delivery-shape decision fixtures

- Covers: `VOC-090-AC-00` through `VOC-090-AC-03`
- Procedure: apply the guidance to synthetic plans for (a) one coherent cross-component
  ...
  coherent diff, and (g) a small diff containing unrelated cleanup.
- Expected: (a), (d), and (f) default to one package/PR unless another documented
  ...
  (g) separates unrelated work regardless of size.
...
- Procedure: run the foundation validator directly and its unit tests, then mutate
  ...
  remove the one-package/one-PR default; state or imply that each task ID owns a PR;
  remove required multiple-PR rationale/overhead markers; and retain an explicitly
...
- Covers: `VOC-090-AC-04`, `VOC-090-AC-06`
- Procedure: from the exact implementation branch and adopted base, run:
  ...
  pnpm exec prettier --check AGENTS.md CLAUDE.md CONTRIBUTING.md .github/pull_request_template.md docs/governance/16-autonomous-development-operating-model.md docs/governance/change-risk-classification.md docs/operations/10-development-workflow.md docs/operations/15-ai-native-product-and-engineering-operating-model.md docs/operations/ruflo-external-orchestration.md docs/templates/change-specification.md specs/README.md specs/templates/change-package
