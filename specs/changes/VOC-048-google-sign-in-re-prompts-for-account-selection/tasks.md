...
## VOC-048-T00 — Confirm whether the app-level session persists as intended, independent of Google's prompt behavior

- Requirement source: `specification.md`'s "What remains unconfirmed" section
- Acceptance criteria: `VOC-048-AC-00`
...
`VOC-048-T01` begins, since `T01`'s scope may need to change depending on
...
## VOC-048-T01 — Stop requesting `prompt=consent` unconditionally on every Google sign-in

- Requirement source: `specification.md`'s objective and "Confirmed finding"
  section; gated on `VOC-048-T00`'s completed finding
- Acceptance criteria: `VOC-048-AC-01`, `VOC-048-AC-02`, `VOC-048-AC-03`,
  `VOC-048-AC-04`
- Tests: `VOC-048-TEST-01`, `VOC-048-TEST-02`, `VOC-048-TEST-03`,
  `VOC-048-TEST-04`
...
`OAuthCallback` unchanged (`VOC-048-AC-03`). Preserve a genuine OAuth
round-trip requirement after explicit sign-out (`VOC-048-AC-02`). Update
...
handled (`VOC-048-AC-04`), not silently dropped. Depends on `VOC-048-T00`'s
