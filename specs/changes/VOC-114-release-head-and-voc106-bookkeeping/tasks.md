...
After adoption, a different builder reconciles exactly the declared existing files,
runs the full deterministic and semantic matrix, obtains distinct exact specialist
and R4 review, rehearses rollback, and hands the exact revision to a separate merge
actor. The task performs no release, settings, deletion, dispatch, deployment, or
