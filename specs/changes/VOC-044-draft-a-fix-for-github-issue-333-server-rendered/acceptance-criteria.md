# VOC-044 — Acceptance Criteria

## VOC-044-AC-00 — api-server.ts forwards the raw incoming Cookie header, not a reconstructed one

- Requirement source: issue #333's "likely area to investigate" and "suggested fix
  direction"
- Tasks: `VOC-044-T00`
- Tests: `VOC-044-TEST-00`
...
- Observable outcome: `apps/web/src/lib/api-server.ts`'s `createServerApiClient()`
  ...
  `VOC-044-TEST-00`'s assertion that the header value forwarded to the injected
  test `fetch` is byte-for-byte identical to a representative multi-cookie raw
  input header.

## VOC-044-AC-01 — Every existing caller of createServerApiClient() still authenticates correctly
...
## VOC-044-AC-02 — A deterministic regression check catches a reintroduced cookie-reconstruction path
