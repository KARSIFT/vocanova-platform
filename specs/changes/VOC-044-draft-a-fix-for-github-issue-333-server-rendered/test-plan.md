...
## VOC-044-TEST-00 — Forwarded Cookie header matches the raw incoming header byte-for-byte

- Covers: `VOC-044-AC-00`
- Preconditions: `VOC-044-T00`'s fix applied to `apps/web/src/lib/api-server.ts`.
- Procedure:
...
- Evidence: `VOC-044-EV-00`

## VOC-044-TEST-01 — Existing callers behave identically post-fix

- Covers: `VOC-044-AC-01`
- Preconditions: `VOC-044-T00`'s fix applied; `apps/web/src/app/onboarding/page.tsx`
...
- Evidence: `VOC-044-EV-01`

## VOC-044-TEST-02 — Regression test fails pre-fix, passes post-fix, runs in CI

- Covers: `VOC-044-AC-02`
- Preconditions: `VOC-044-T01`'s test implemented.
- Procedure:
  1. Run the new test against a copy of `api-server.ts` with `VOC-044-T00`'s fix
...
beyond `VOC-044-TEST-01` step 3's confirmation that the `401`/no-cookie redirect
