...
Preconditions: `VOC-044-T00` and `VOC-044-T01` implemented, independently
verified against the exact revision to be released, and
`VOC-044-AC-00`/`01`/`02` each satisfied with real evidence (not asserted) per
...
(`VOC-044-TEST-00`) demonstrates the divergence and its correction before
...
pre-merge by `VOC-044-T01`'s regression test, but also a valid post-deploy
...
matches the same content it had before this package's fix (i.e. the pre-VOC-044
