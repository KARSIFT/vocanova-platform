...
1. `VOC-044-T00`: in `apps/web/src/lib/api-server.ts`, replace the `cookies()`
   ...
   `VOC-044-TEST-00`/`VOC-044-TEST-01`'s procedures.
2. `VOC-044-T01`: add a regression test (new file, e.g.
   ...
   implementer should verify this by temporarily reverting `VOC-044-T00`'s change
...
git diff --check
pnpm validate   # or the narrower relevant script, once VOC-044-T01's test exists
```

Plus this package's own `VOC-044-TEST-00`/`01`/`02` procedures.
...
confirm `VOC-044-AC-00`/`01`/`02` are each satisfied with real evidence (not
...
(`VOC-044-T01`'s regression test should catch this before merge, but a rollback
