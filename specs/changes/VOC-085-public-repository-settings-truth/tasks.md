...
## VOC-085-T00 — Record the current public settings snapshot
...
read-only values, observation date, source commands/API surface, and explicit
...
- Requirements: `VOC-085-D02`, `VOC-085-D04`, `VOC-085-D05`
...
delivery guide, and DOC-16 wording/metadata as applicable. Current public settings,
configured Actions hardening, absent/disabled controls, historical VOC-080 evidence,
and prospective/held mature protections must be clearly separated. Update the DOC-16
amendment history when its current posture changes. Do not rewrite historical records
or activate any desired setting.
...
must validate required current-state fields and
...
text. No GitHub API write, merge, comment, close, dispatch, settings mutation, or
...
Run proportional governance/risk/diff validation, exact-SHA independent review,
applicable hosted checks, and a disposable reverse-order repository rollback rehearsal.
After a normal merge into `develop` and passing post-merge checks, an accountable
operator may close #119 with repository-only evidence. Keep all VOC-080 holds open and
do not mutate settings, deploy, promote `main`, or delete branches.
