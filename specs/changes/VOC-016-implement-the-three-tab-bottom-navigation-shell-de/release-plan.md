...
Because this draft leaves `automatic_merge_allowed: false`, no automatic merge is
asserted; the merge decision is made by a human/CI at implementation time under
the merge-gate policy then in force, not by this document.
...
nothing depends on it yet. Owner: founder. Last-known-good reference: `develop` at
this package's (adoption-time re-pinned) `base_sha`.
...
plus the same manual `develop`-merge closure step the recent VOC-0xx packages
used. Production deployment and autonomous release remain disabled.
