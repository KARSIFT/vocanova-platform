...
- **Accessibility:** in scope and central. The package is specified to meet
  ...
  targets — see `acceptance-criteria.md` `VOC-016-AC-02`/`AC-03`. Honest
  limitation: with no web test runner yet (`VOC-016-DEP-05`), AA conformance is
  ...
  automated coverage is a flagged follow-up (`VOC-016-D08`), not a silently
  skipped requirement.

## Risks, dependencies, and evidence

- `VOC-016-R00` (Low): The nav is a persistent shell element rendered on every
  ...
  per-item accessibility checklist (`VOC-016-AC-02`) and reviewer inspection.
- `VOC-016-R01` (Medium, scoping): "navigation shell only" is easy to overrun
...
- `VOC-016-R02` (design decision, not correctness): the Journey tab route name is
  a genuine DOC-03-vs-DOC-08 reconciliation, recorded not silently resolved
  (`VOC-016-D01`). `/journey` is proposed; `/discover` is the DOC-08 alternative.
  Only one route folder + one nav href depend on the choice.
- `VOC-016-R03` (accessibility semantic risk): as the first user-facing UI, this
  ...
  `CLAUDE.md`, and the automated-a11y gap is acknowledged (`VOC-016-D08`).
...
- `VOC-016-DEP-02`: Base state (`base_sha`) is pinned to the drafting-time
  `develop` head (`f5b920f8b6aa31565b66b03972b73ed6f0360c7b`) and must be re-pinned
  to the then-current `develop` head at adoption.
- `VOC-016-DEP-03`: Depends on the existing Next.js web foundation on `develop`
...
- `VOC-016-DEP-04`: Journey tab route name open for founder decision
  (`VOC-016-D01`).
- `VOC-016-DEP-05`: No `apps/web` test runner exists; automated a11y/component
  testing is a required follow-up, not part of this shell.
- `VOC-016-EV-00`..`VOC-016-EV-03`: CI run output (lint/typecheck/build/format)
