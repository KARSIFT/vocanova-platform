...
normal non-author merge, and a separately authorized first staging dispatch. Task
identity is traceability only and does not create external authority.

The builder must stop on scope expansion, unexpected settings state, repository
secret duplication, broader token scope, paid-plan requirement, production drift,
non-synthetic data, ambiguous rollback versions, or any reviewer blocker.
