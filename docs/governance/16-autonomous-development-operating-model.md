...
any environment. VOC-094 through VOC-099 remain immutable historical evidence, but
VOC-100 prospectively supersedes their future five-record binder, per-dispatch token
recreation, and per-dispatch PR/package requirements. Standard staging delivery now
depends on the `cloudflare-staging` GitHub environment, branch-bound manual dispatch,
same-run required checks, and a separately authorized environment/secrets/delegation
record. VOC-100 PR2 records that settings truth; no merge by itself dispatches,
deploys, or changes production. Production and learner-data holds remain unchanged. See
...
| Staging deployment, health checks                                                               | Exact Phase-1 Workers/D1/domains and rollback/public-smoke evidence exist. VOC-100 supersedes the future five-record binder; PR2 records the separately authorized `cloudflare-staging` settings/secrets/delegation truth. Ordinary dispatches and token revocations or replacements need no package or PR. |
