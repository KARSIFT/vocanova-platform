...
record followed by one immediate documentation-only settings-truth PR. No merge by
itself creates the environment, enters secrets, dispatches, deploys, or accepts the
shared-identity receipt risk. Production and learner-data holds remain unchanged. See
